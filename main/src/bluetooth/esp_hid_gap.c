...
static void add_bt_scan_result(esp_bd_addr_t bda, esp_bt_cod_t *cod, esp_bt_uuid_t *uuid, uint8_t *name,
...
static void handle_bt_device_result(struct disc_res_param *disc_res)
{
    ...
    for(int i = 0; i < disc_res->num_prop; i++)
    {
        ...
        else if(prop->type == ESP_BT_GAP_DEV_PROP_COD)
        {
            ...
            GAP_DBG_PRINTF("major: %s, minor: %d, service: 0x%03x", esp_hid_cod_major_str(cod->major), cod->minor,
                           cod->service);
        }
